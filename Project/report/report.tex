...
The average value for the plaquette during thermalisation is plotted in fig.~\ref{fig:comparisoncreutz} for SU(2)-matrices and in fig.~\ref{fig:plaquettethermsu3} for SU(3). In order to be able to compare our results to the different definition of the plaquette of \citet{creutzsu2}, we have plotted $1-\langle P\rangle$. Per step, we did 10 Metropolis-updates of each link in the lattice, and the plaquette was calculated directly after one update. The contribution for each link $U_\mu(x)$ was calculated as $\sum_{\mu>\nu}P_{\mu\nu}(x)$.
...
After the thermalisation, we did our measurements on the lattice. All measurements were done after 100 thermalisation steps, with the number of measurements between $1024$ and $8192$ per $\beta$. The analysis was done with a binsize of $32$ and $200$ bootstrapsamples. 
