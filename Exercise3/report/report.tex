...
The convergence of our leapfrog algorithm is shown in fig.~\ref{fig:converge}. Qualitatively, it behaves the same as the expected one on the sheet: We start with a high value of $\Delta$, and it gets smaller with each step we add to the algorithm. Qunatitatively, our values of $\Delta$ are about two orders of magnitude larger than those on the sheet, but we do not know what was simulated in the example on the sheet, so this most likely explains any differences.
...
The analytical expectation and our measurements for the magnetization are plotted in fig.~\ref{fig:magnetization}. We expect the magnetization to be about $0.5$ for $J=0.2$, and then rise monotonuously. This rise gets less steep for growing $J$, and does not reach $m=1$. For smaller $N$, we expect the magnetization to be smalelr and its rise to be less steep than for larger $N$, however the diatnde between the lines decreases for growing $N$.
