...
\section{Simulation}
Using gsl\_rng generator with Mersenne-Twister\cite{gsldoc} and gsl\_block \cite{gsldoc_blk}
Each set of wanted h and N(length) parameter sets gets iterated through.
In each iteration we generate a specific amount of  random spin configurations, with the wanted length N.
Then we calculate the boltzman weight and the mean of the spins in each configuration. These values get stored and are also used to calculate the expectation value of the magnetization for the given parameter set and the partition function. At the end of each iteration the stored boltzman weights an mean values of the spins get used to calculate the variance of our result. These values get saved into a file. 

From this file the data points get plotted using gnuplot, the analytical solution got implemented directly into gnuplot.

\section{Results}
\label{sec:results}
\subsection{Varying h}
The results of the simulations for varying $h$ and fixed $N$ at $2^{21}$ samples per measurement,as well as the expectations from sec.~\ref{subsec:expectationmagnetization} are plotted in fig.~\ref{fig:magvarh}. We expect the magnetization to be zero for $h=0$, and for $h>0$ we expect the magnetization to be one for  low temperatures and to fall for larger temperatures. This fall is expected to be steeper for smaller $h$, and the slope first gets steeper for higher temperatures, but then gets smaller again and the graphs for different values of $h$ get closer again. For larger temperatures, the magnetization converges to zero. For $h<0$, the absolute value of the magnetization behaves identically, but the magnetization is negative. For $h\neg 0$ our measurements follow the expectation pretty weel, for $N=16$ and $2^{21}$ sampled configurations almost all points lie on the expected curve and those few that so not are really close. The error on the measurement grows with growing temperature. For $h=0$, all points at higher temperatures lie on the curve, but there are some at small temperatures that are pretty far off. However, the errors get smaller with rising temperatures. The far off-points would probably be improved with more measurements.
